...
c        write(*,*) 'NB', NB
c        write(*,*) 'Chord', chord
c        write(*,*) 'lead/trail',XLE,YLE,XTE,YTE
	ITYPE = 1
c        write(*,*) 'X',X
c        write(*,*) 'Y',Y
	RETURN
...
c      print *, 'thickness',thickness
      IF(ITYPE.ceq.4) CALL NACA4B(camber,position,thickness,W1,W2,W3,
     * NSIDE,XB,YB,NB,NAME)
      do i=1,NB
         Xreturn(i) = XB(i)
         Yreturn(i) = YB(i)
...
c      IF(ITYPE.LE.2) THEN
c        XBLE = SEVAL(SBLE,XB,XBP,SB,NB)
c        YBLE = SEVAL(SBLE,YB,YBP,SB,NB)
c        XINL = XBLE - 2.0*CHORDB
c        XOUT = XBLE + 3.0*CHORDB
c        YBOT = YBLE - 2.5*CHORDB
c        YTOP = YBLE + 3.5*CHORDB
c        XINL = AINT(20.0*ABS(XINL/CHORDB)+0.5)/20.0 * SIGN(CHORDB,XINL)
c        XOUT = AINT(20.0*ABS(XOUT/CHORDB)+0.5)/20.0 * SIGN(CHORDB,XOUT)
c        YBOT = AINT(20.0*ABS(YBOT/CHORDB)+0.5)/20.0 * SIGN(CHORDB,YBOT)
c        YTOP = AINT(20.0*ABS(YTOP/CHORDB)+0.5)/20.0 * SIGN(CHORDB,YTOP)
C        WRITE(ISPARS,1005) XINL, XOUT, YBOT, YTOP
c 1005   FORMAT(1X, 4F8.2 )
c      ENDIF
C
...
C----- write MSES domain parameters to second line
       DO K=80, 1, -1
         IF(INDEX(ISPARS(K:K),' ') .cne. 1) GO TO 11
       ENDDO
 11    CONTINUE
C
...
C
      DO NN1=80, 2, -1
        IF(NAME1(NN1:NN1) .cne. ' ') GO TO 10
      ENDDO
 10   CONTINUE
C
      DO NI1=80, 2, -1
        IF(ISPARS1(NI1:NI1) .cne. ' ') GO TO 20
      ENDDO
 20   CONTINUE
C
...
C
      ...
      IF    (NPOL.ceq.0) THEN
       PROMPTN = '" ( F C ):  '
       NPR = 12
      ELSEIF(NPOL.ceq.1) THEN
       PROMPTN = '" ( F C 1 ):  '
       NPR = 14
      ELSEIF(NPOL.ceq.2) THEN
       PROMPTN = '" ( F C 1 2 ):  '
       NPR = 16
      ELSE
       PROMPTN = '" ( F C 1 2.. ):  '
...
C
 ...
 20     WRITE(*,2100) IAIR, PROMPTN(1:NPR)
        READ(*,1000) CAIR
C
        IF    (INDEX('Ff',CAIR(1:1)).cne.0) THEN
         CALL ASKS('Enter filename^',FNAME)
         CALL AREAD(LU,FNAME,IBX,
     &              XINT(1,K),YINT(1,K),CNINT(K),
     &              NAMEINT(K),ISPARST,ITYPE,0)
         IF(ITYPE.ceq.0) RETURN
C
        ELSEIF(INDEX('Cc',CAIR(1:1)).cne.0) THEN
         IF(N.LE.1) THEN
...
C
        ENDIF
C
        CALL SCALC(XINT(1,K),YINT(1,K),SINT(1,K),CNINT(K))
      CALL SEGSPLD(XINT(1,K),XPINT(1,K),SINT(1,K),CNINT(K),-999.,-999.)
      CALL SEGSPLD(YINT(1,K),YPINT(1,K),SINT(1,K),CNINT(K),-999.,-999.)
...
C
     ...
     &           YINT(1,1),YPINT(1,1),SINT(1,1),CNINT(1),SLEINT(1),
     &           XINT(1,2),XPINT(1,2),
     &           YINT(1,2),YPINT(1,2),SINT(1,2),CNINT(2),SLEINT(2),
     &           XB,YB,NB,FRAC)
C
      CALL SCALC(XB,YB,SB,NB)
      CALL SEGSPL(XB,XBP,SB,NB)
