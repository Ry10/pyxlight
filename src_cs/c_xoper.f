...
C
      IF(LM) THEN
       IF(MATYP.ceq.1) WRITE(*,1100) MINF1
       IF(MATYP.ceq.2) WRITE(*,1100) MINF1, ' / sqrt(CL)'
       IF(MATYP.ceq.3) WRITE(*,1100) MINF1, ' / CL'
      ENDIF
C
      IF(LR) THEN
       IF(RETYP.ceq.1) WRITE(*,1200) INT(REINF1)
       IF(RETYP.ceq.2) WRITE(*,1200) INT(REINF1), ' / sqrt(CL)'
       IF(RETYP.ceq.3) WRITE(*,1200) INT(REINF1), ' / CL'
      ENDIF
C
      RETURN
C
...
C
      NAMDEF = NAME(1:NNAME)
C
      IF((KBRACK1.cne.0) .AND. (
     &   KBRACK2.cne.0) .AND. KBRACK2-KBRACK1.GT.1) THEN
...
C
      IF(COMAND.ceq.'    ') RETURN
      IF(COMAND.ceq.'?   ') GO TO 5
      IF(COMAND.ceq.'SHOW') GO TO 10
      IF(COMAND.ceq.'XTR ') GO TO 40
      IF(COMAND.ceq.'N   ') GO TO 50
      IF(COMAND.ceq.'VACC') GO TO 70
      IF(COMAND.ceq.'INIT') GO TO 80
C
      WRITE(*,1000) COMAND
      GO TO 500
C
...
C-------- if Mach is OK, go do next Newton iteration
          IF((MATYP.ceq.1) .OR. (MINF.ceq.0.0) .OR. (MINF_CLM.cne.0.0))
     &    GO TO 91
C
          RLX = 0.5*RLX
   90   CONTINUE
...
C---- Newton iteration for entire BL solution
      IF(NITER.ceq.0) CALL ASKI('Enter number of iterations^',NITER)
C      WRITE(*,*)
...
c        WRITE (*,*) 'Check some stuff: X',X
c        WRITE (*,*) 'Check some stuff: Y',Y
c        WRITE (*,*) 'Check some stuff: GAM',GAM
c        WRITE (*,*) 'Check some stuff: UEDG',UEDG
	CALL SETBL
